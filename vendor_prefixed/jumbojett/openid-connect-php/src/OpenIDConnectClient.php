 ...
 *
 */
namespace OneId_Vendor\Jumbojett;
 ...
 */
if (!\class_exists('OneId_Vendor\\phpseclib\\Crypt\\RSA') && !\class_exists('OneId_Vendor\\Crypt_RSA')) {
...
class OpenIDConnectClient
{
    ...
    public function signOut($idToken, $redirect)
    {
        ...
        if ($redirect === null) {
            ...
            $signout_params = array('id_token_hint' => $idToken, 'post_logout_redirect_uri' => $redirect);
        }
    ...
    private function requestAuthorization()
    {
        ...
        $auth_params = \array_merge($this->authParams, array('response_type' => $response_type, 'redirect_uri' => $this->getRedirectURL(), 'client_id' => $this->clientID, 'nonce' => $nonce, 'state' => $state, 'scope' => 'openid'));
        ...
        if (!empty($ccm) && \in_array($this->getCodeChallengeMethod(), $this->getProviderConfigValue('code_challenge_methods_supported'))) {
            ...
            if (!empty($this->pkceAlgs[$this->getCodeChallengeMethod()])) {
                $codeChallenge = \rtrim(\strtr(\base64_encode(\hash($this->pkceAlgs[$this->getCodeChallengeMethod()], $codeVerifier, \true)), '+/', '-_'), '=');
            } else {
                $codeChallenge = $codeVerifier;
            }
            $auth_params = \array_merge($auth_params, array('code_challenge' => $codeChallenge, 'code_challenge_method' => $this->getCodeChallengeMethod()));
        }
    ...
    public function requestClientCredentialsToken()
    {
        ...
        $post_data = array('grant_type' => $grant_type, 'client_id' => $this->clientID, 'client_secret' => $this->clientSecret, 'scope' => \implode(' ', $this->scopes));
    ...
    public function requestResourceOwnerToken($bClientAuth = FALSE)
    {
        ...
        $headers = [];
        $grant_type = 'password';
        $post_data = array('grant_type' => $grant_type, 'username' => $this->authParams['username'], 'password' => $this->authParams['password'], 'scope' => \implode(' ', $this->scopes));
        //For client authentication include the client values
        if ($bClientAuth) {
            $token_endpoint_auth_methods_supported = $this->getProviderConfigValue('token_endpoint_auth_methods_supported', ['client_secret_basic']);
            if (\in_array('client_secret_basic', $token_endpoint_auth_methods_supported, \true)) {
    ...
    protected function requestTokens($code)
    {
        ...
        $token_params = array('grant_type' => $grant_type, 'code' => $code, 'redirect_uri' => $this->getRedirectURL(), 'client_id' => $this->clientID, 'client_secret' => $this->clientSecret);
        ...
        if (!empty($ccm) && !empty($cv)) {
            $headers = [];
            unset($token_params['client_secret']);
            $token_params = \array_merge($token_params, array('client_id' => $this->clientID, 'code_verifier' => $this->getCodeVerifier()));
        }
        // Convert token params to string format
    ...
    public function refreshToken($refresh_token)
    {
        ...
        $token_params = array('grant_type' => $grant_type, 'refresh_token' => $refresh_token, 'client_id' => $this->clientID, 'client_secret' => $this->clientSecret, 'scope' => \implode(' ', $this->scopes));
        # Consider Basic authentication if provider config is set this way
        if (\in_array('client_secret_basic', $token_endpoint_auth_methods_supported, \true)) {
            $headers = ['Authorization: Basic ' . \base64_encode(\urlencode($this->clientID) . ':' . \urlencode($this->clientSecret))];
            unset($token_params['client_secret']);
    ...
    private function verifyRSAJWTsignature($hashtype, $key, $payload, $signature, $signatureType)
    {
        ...
        if (\class_exists('OneId_Vendor\\Crypt_RSA', \false)) {
            $rsa = new Crypt_RSA();
            ...
            $rsa = new \OneId_Vendor\phpseclib\Crypt\RSA();
            $rsa->setHash($hashtype);
            if ($signatureType === 'PSS') {
                $rsa->setMGFHash($hashtype);
            }
            $rsa->loadKey($public_key_xml, \OneId_Vendor\phpseclib\Crypt\RSA::PUBLIC_FORMAT_XML);
            $rsa->signatureMode = $signatureType === 'PSS' ? \OneId_Vendor\phpseclib\Crypt\RSA::SIGNATURE_PSS : \OneId_Vendor\phpseclib\Crypt\RSA::SIGNATURE_PKCS1;
        }
    ...
    public function verifyJWTsignature($jwt)
    {
        ...
        switch ($header->alg) {
            ...
            case 'RS512':
                ...
                $verified = $this->verifyRSAJWTsignature($hashtype, $this->get_key_for_header($jwks->keys, $header), $payload, $signature, $signatureType);
                break;
            case 'HS256':
            case 'HS512':
            case 'HS384':
    ...
    public function register()
    {
        ...
        $send_object = (object) \array_merge($this->registrationParams, array('redirect_uris' => array($this->getRedirectURL()), 'client_name' => $this->getClientName()));
    ...
    {
        return \class_exists('OneId_Vendor\\phpseclib\\Crypt\\RSA') || \class_exists('OneId_Vendor\\Crypt_RSA');
    }
    /**
